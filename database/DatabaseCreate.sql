...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`contact_data` (
  ...
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `payment_credentials` VARCHAR(255) NULL,
  PRIMARY KEY (`contact_data_id`),
  UNIQUE INDEX `address_id_unique` (`contact_data_id` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`customer` (
  ...
  `payment_method` ENUM('paypal', 'debit', 'bill') NULL DEFAULT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  `contact_data_id` INT(11) NOT NULL,
  `account_id` INT(11) NOT NULL,
  `is_business_customer` TINYINT(4) NOT NULL,
  PRIMARY KEY (`customer_id`, `contact_data_id`),
  UNIQUE INDEX `cust_id_unique` (`customer_id` ASC) VISIBLE,
  INDEX `fk_customer_contact_data1_idx` (`contact_data_id` ASC) VISIBLE,
  INDEX `fk_customer_account_details1_idx` (`account_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`role` (
  ...
  `is_enabled_to_manage_employee_data` TINYINT(4) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`employee` (
  ...
  UNIQUE INDEX `emp_no_unique` (`emp_no` ASC) VISIBLE,
  INDEX `fk_employee_role1_idx` (`given_role` ASC) VISIBLE,
  INDEX `fk_employee_account_details1_idx` (`account_id` ASC) VISIBLE,
  UNIQUE INDEX `account_id_UNIQUE` (`account_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`booking` (
  ...
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `pricing` DECIMAL(7,2) NULL,
  PRIMARY KEY (`booking_no`),
  UNIQUE INDEX `booking_no_unique` (`booking_no` ASC) VISIBLE,
  INDEX `fk_booking_employee1_idx` (`emp_no` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`conference_room` (
  ...
  UNIQUE INDEX `conference_room_id_UNIQUE` (`conference_room_id` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`hotel_room` (
  ...
  UNIQUE INDEX `hotel_room_id_UNIQUE` (`hotel_room_id` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`room` (
  ...
  `category` ENUM('SINGLE', 'DOUBLE', 'SUITE', 'BIGGROUP', 'SMALLGROUP') NOT NULL,
  `price_per_unit` DECIMAL(10,0) NOT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `hotel_room_id` INT(11) NULL,
  `conference_room_id` INT(11) NULL,
  `room_type` ENUM('HOTELROOM', 'CONFERENCEROOM') NOT NULL,
  PRIMARY KEY (`room_no`),
  UNIQUE INDEX `room_no_UNIQUE` (`room_no` ASC) VISIBLE,
  INDEX `fk_room_hotel_room1_idx` (`hotel_room_id` ASC) VISIBLE,
  INDEX `fk_room_conference_room1_idx` (`conference_room_id` ASC) VISIBLE,
  UNIQUE INDEX `conference_room_conference_room_id_UNIQUE` (`conference_room_id` ASC) VISIBLE,
  UNIQUE INDEX `hotel_room_hotel_room_id_UNIQUE` (`hotel_room_id` ASC) VISIBLE,
  CONSTRAINT `fk_room_hotel_room1`
    FOREIGN KEY (`hotel_room_id`)
    REFERENCES `bh_room_db`.`hotel_room` (`hotel_room_id`)
    ON DELETE SET NULL
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_room_conference_room1`
    FOREIGN KEY (`conference_room_id`)
    REFERENCES `bh_room_db`.`conference_room` (`conference_room_id`)
    ON DELETE SET NULL
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`room_has_booking` (
  ...
  INDEX `fk_room_has_booking_booking1_idx` (`booking_id` ASC) VISIBLE,
  INDEX `fk_room_has_booking_room1_idx` (`room_no` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`booking_request` (
  `booking_request_id` INT(11) NOT NULL AUTO_INCREMENT,
  `customer_id` INT(11) NOT NULL,
  `room_category` ENUM('SINGLE', 'DOUBLE', 'SUITE', 'BIGGROUP', 'SMALLGROUP') NOT NULL,
  `booking_type` ENUM('ConferenceRoomBooking', 'HotelRoomBooking') NOT NULL,
  ...
  UNIQUE INDEX `booking_no_unique` (`booking_request_id` ASC) VISIBLE,
  INDEX `fk_booking_customer1_idx` (`customer_id` ASC) VISIBLE,
  CONSTRAINT `fk_booking_customer10`
    FOREIGN KEY (`customer_id`)
    REFERENCES `bh_room_db`.`customer` (`customer_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
