...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`employee` (
  ...
  `empName` VARCHAR(45) NOT NULL DEFAULT 'Maximilian',
  `givenRole` VARCHAR(45) NOT NULL DEFAULT 'Kundenverwalter',
  `accountId` INT NULL,
  `createdAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdate` TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`empNo`),
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`booking` (
  ...
  INDEX `fk_Booking_Employee1_idx` (`empNo` ASC),
  INDEX `fk_Booking_Customer1_idx` (`customerID` ASC),
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`room_has_booking` (
  ...
  INDEX `fk_Room_has_Booking_Booking1_idx` (`bookingId` ASC),
  INDEX `fk_Room_has_Booking_Room1_idx` (`roomNo` ASC),
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`hotelroom` (
  ...
  `roomNo` INT NOT NULL,
  INDEX `fk_hotelroom_room1_idx` (`roomNo` ASC),
...
CREATE TABLE IF NOT EXISTS `bh_room_db`.`conferenceroom` (
  ...
  `hasTV` TINYINT NULL,
  `roomNo` INT NOT NULL,
  INDEX `fk_conferenceroom_room1_idx` (`roomNo` ASC),
